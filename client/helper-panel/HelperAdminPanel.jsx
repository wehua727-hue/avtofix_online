...
import { storesAPI, productsAPI, ordersAPI, uploadsAPI } from "@/services/api";
...
import { Package, Store, Users, ArrowLeft, Home, Plus, Trash2, ChevronDown, Pencil, X, Search, Menu } from "lucide-react";
import { toast } from "sonner";
import { formatPrice } from "@/utils/currency";

// Компонент для управления помощниками (используется если есть разрешение helpers)
function HelpersManagement() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const { managerAPI } = await import("@/services/api");
        const hs = await managerAPI.getHelpers();
        setHelpers(hs);
      } catch (e) {
        console.error(e);
        toast.error("Yordamchilarni yuklashda xatolik");
      } finally {
        setLoading(false);
      }
    };
  ...
  const toggle = async (id, key, value) => {
    ...
    try {
      ...
      setHelpers((arr) => arr.map(h => (h.id === helperId || h._id === helperId) ? updated : h));
    } catch (e) { 
  ...
  const handleRemove = async (id) => {
    ...
    try {
      ...
      setHelpers((arr) => arr.filter(h => h.id !== helperId && h._id !== helperId));
    } catch (e) {
      toast.error(e.message || "Xatolik");
    }
  };
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        ...
        <Button 
          size="sm" 
          className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-red-500 via-rose-600 to-orange-500 text-white shadow-lg shadow-red-500/30 hover:shadow-xl transition-all"
      ...
      {loading ? (
        ...
        <div className="text-gray-600 dark:text-white/80">Hozircha yordamchilar yo'q</div>
      ) : (
        <div className="space-y-2">
          {helpers.map((h) => {
            if (h.id === currentUser?.id || h._id === currentUser?.id) {
              return null; // Не показываем себя
            }
            const helperId = h.id || h._id;
            return (
      ...
      {/* Assign Helper Modal */}
      <Dialog open={assignOpen} onOpenChange={(v)=>{ setAssignOpen(v); if(!v){ setSelectedUser(null); setPerm({products:false,orders:false,helpers:false}); } }}>
            ...
            <div>
              ...
              <div className="relative mt-1">
                <select
                  ...
                  className="w-full appearance-none bg-white dark:bg-gray-800 border border-gray-300 dark:border-white/15 text-gray-900 dark:text-white rounded-lg px-3 pr-10 h-10 focus:outline-none focus:ring-2 focus:ring-rose-500/40 focus:border-rose-500/40 [&>option]:bg-white [&>option]:dark:bg-gray-800 [&>option]:text-gray-900 [&>option]:dark:text-white"
                >
                  <option value="" disabled className="bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400">Tanlang...</option>
                  {usersLoading ? (
                    <option className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">Yuklanmoqda...</option>
                  ) : (
                    (userCandidates||[])
                      .filter(u=>u.role==='user')
                      .sort((a,b)=> (a.name||'').localeCompare(b.name||'') || String(a.phone||'').localeCompare(String(b.phone||'')))
                      .map(u=> (
                        <option key={u.id} value={u.id} className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{u.name || "Noma'lum"} — +998{u.phone}</option>
                      ))
                  )}
                </select>
...
export default function HelperAdminPanel() {
  ...
  const perms = currentUser?.helperPermissions || { products: false, orders: false, helpers: false };
  const helperStoreId = currentUser?.managerOfShop || null;
  ...
  useEffect(() => {
    const init = async () => {
      try {
        setLoading(true);
        if (!helperStoreId) {
          toast.error("Yordamchiga magazin biriktirilmagan");
          return;
        }
        const s = await storesAPI.getById(helperStoreId);
        setStore(s);
        // Загружаем продукты только если есть разрешение на продукты
        if (perms.products) {
          try {
            const prods = await productsAPI.getAll({ storeId: helperStoreId, includeInactive: true, adminPanel: true });
            setProducts(prods);
          } catch (err) {
            // Если нет разрешения, просто не загружаем
            if (err.message?.includes('ruxsat') || err.message?.includes('permission')) {
              setProducts([]);
            } else {
              throw err;
            }
        ...
        console.error(e);
        // Не показываем ошибку, если это просто отсутствие разрешения
        if (!e.message?.includes('ruxsat') && !e.message?.includes('permission')) {
    ...
    init();
  }, [helperStoreId, perms.products, active]);

  const tabs = useMemo(() => {
    const t = [];
    // Если есть разрешение на продукты, показываем вкладку Mahsulotlar
    if (perms.products) {
      t.push({ key: "products", label: "Mahsulotlar", icon: <Store className="h-4 w-4" /> });
    }
    if (perms.orders) {
      t.push({ key: "orders", label: "Buyurtmalar", icon: <Package className="h-4 w-4" /> });
    }
    if (perms.helpers) {
      t.push({ key: "helpers", label: "Yordamchilar", icon: <Users className="h-4 w-4" /> });
    }
    // Если нет ни одной вкладки, показываем пустое сообщение
    if (t.length === 0) {
      t.push({ key: "no-access", label: "Ruxsat yo'q", icon: <Package className="h-4 w-4" /> });
    }
    return t;
  }, [perms]);
  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-[#06080f] text-gray-900 dark:text-white">
      <div className="flex min-h-screen">
        ...
        <aside className={`fixed inset-y-0 left-0 z-50 w-64 transform bg-white dark:bg-[#0a0c12] border-r border-gray-200 dark:border-white/10 transition-transform duration-300 lg:relative lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex h-full flex-col p-4">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Yordamchi paneli</h2>
        ...
        <div className="flex-1 flex flex-col min-h-screen">
          ...
          <div className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-gray-200 dark:border-white/10 bg-white/80 dark:bg-[#0a0c12]/80 backdrop-blur-xl px-4 py-3">
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-600 dark:text-white/60 hidden sm:inline truncate max-w-[150px]">{currentUser?.name || "Yordamchi"}</span>
          ...
          <section className="flex-1 p-4 sm:p-6 overflow-y-auto">
            {loading ? (
              ...
              <div className="rounded-xl border border-dashed border-gray-300 dark:border-white/15 bg-gray-50 dark:bg-white/5 p-12 text-center">
                <Package className="h-12 w-12 mx-auto mb-4 text-gray-400 dark:text-white/40" />
                <p className="text-gray-600 dark:text-white/80 mb-2">Sizga hech qanday bo'limga ruxsat berilmagan</p>
                <p className="text-xs text-gray-500 dark:text-white/60">Menedjerdan ruxsat so'rang</p>
              </div>
            ) : active === "products" ? (
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
                  <CardTitle className="text-xl font-bold text-gray-900 dark:text-white">Mahsulotlar</CardTitle>
                  {perms.products && (
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <div className="relative flex-1 sm:flex-none sm:w-64">
                        ...
                        <Input
                          ...
                          className="pl-9 h-9 rounded-xl bg-gray-50 dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-900 dark:text-white placeholder:text-gray-400"
                        />
                      </div>
                      <Button 
                        size="sm" 
                        className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-red-500 via-rose-600 to-orange-500 text-white shadow-lg shadow-red-500/30 hover:shadow-xl transition-all h-9 px-4"
                        onClick={() => {
                          setEditingProduct(null);
                          setProductForm({ name:"", price:"", category:"", categoryId:"", description:"", stockCount:"", images:[], imagePreview:"" });
                ...
                {!perms.products ? (
                  <div className="rounded-xl border border-dashed border-gray-300 dark:border-white/15 bg-gray-50 dark:bg-white/5 p-12 text-center">
                    <Package className="h-12 w-12 mx-auto mb-4 text-gray-400 dark:text-white/40" />
                    <p className="text-gray-600 dark:text-white/80 mb-2">Mahsulotlar bo'limiga ruxsat yo'q</p>
                    <p className="text-xs text-gray-500 dark:text-white/60">Menedjerdan ruxsat so'rang</p>
                  </div>
                ) : products.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-gray-300 dark:border-white/15 bg-gray-50 dark:bg-white/5 p-12 text-center">
                    <Store className="h-12 w-12 mx-auto mb-4 text-gray-400 dark:text-white/40" />
                    <p className="text-gray-600 dark:text-white/80">Hozircha mahsulotlar qo'shilmagan</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {products
                      ...
                      .map((p) => {
                        ...
                        return (
                          <div key={p._id} className="group rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 overflow-hidden shadow-sm dark:shadow-none hover:shadow-lg hover:border-red-500/50 dark:hover:border-red-500/50 transition-all">
                            <div className="aspect-square w-full overflow-hidden bg-gray-100 dark:bg-white/5">
                              {img ? (
                                <img src={img} alt={p.name || "product"} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300" />
                              ) : (
                                <div className="h-full w-full flex items-center justify-center">
                                  <Store className="h-10 w-10 text-gray-400 dark:text-white/30" />
                                </div>
                              )}
                            </div>
                            <div className="p-3 space-y-2">
                              <div className="font-semibold text-sm text-gray-900 dark:text-white line-clamp-2 leading-tight">{p.name}</div>
                              <div className="text-sm font-bold text-red-600 dark:text-red-400">{formatPrice(p.price)} {p.currency || "so'm"}</div>
                              {typeof p.stockCount === 'number' && (
                                <div className="text-xs text-gray-500 dark:text-white/70">Omborda: {p.stockCount}</div>
                              )}
                              {perms.products && (
                                <div className="flex items-center gap-2 pt-2">
                                  <Button 
                                    size="sm" 
                                    variant="secondary" 
                                    className="flex-1 rounded-lg bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/15 text-gray-900 dark:text-white text-xs h-8"
                                    onClick={() => {
                                      setEditingProduct(p);
                                      setProductForm({
                                        name: p.name || "",
              ...
              <div>
                {!perms.orders ? (
                  <div className="rounded-xl border border-dashed border-gray-300 dark:border-white/15 bg-gray-50 dark:bg-white/5 p-12 text-center">
                    <Package className="h-12 w-12 mx-auto mb-4 text-gray-400 dark:text-white/40" />
                    <p className="text-gray-600 dark:text-white/80 mb-2">Buyurtmalar bo'limiga ruxsat yo'q</p>
                    <p className="text-xs text-gray-500 dark:text-white/60">Menedjerdan ruxsat so'rang</p>
                  </div>
                ) : (
                  <>
              ...
              <div>
                {!perms.helpers ? (
                  <div className="rounded-xl border border-dashed border-gray-300 dark:border-white/15 bg-gray-50 dark:bg-white/5 p-12 text-center">
                    <Users className="h-12 w-12 mx-auto mb-4 text-gray-400 dark:text-white/40" />
                    <p className="text-gray-600 dark:text-white/80 mb-2">Yordamchilar bo'limiga ruxsat yo'q</p>
                    <p className="text-xs text-gray-500 dark:text-white/60">Menedjerdan ruxsat so'rang</p>
                  </div>
                ) : (
                  <>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {productForm.images.map((url, idx)=>(
                    <img key={idx} src={url} alt="preview" className="h-16 w-16 rounded-lg object-cover border border-gray-200 dark:border-white/10" />
                  ))}
                </div>
            ...
            <DialogFooter className="mt-4">
              ...
              <Button disabled={creating} className="rounded-xl bg-gradient-to-r from-red-500 via-rose-600 to-orange-500" onClick={async ()=>{
                ...
                try{
                  ...
                  toast.error(err.message || (editingProduct ? "Mahsulotni yangilashda xatolik" : "Mahsulotni qo'shishda xatolik"));
                }finally{
                  setCreating(false);
                }
              }}>
                {creating ? (editingProduct ? "Yangilanmoqda..." : "Qo'shilmoqda...") : (editingProduct ? "Yangilash" : "Qo'shish")}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      ...
      {/* Variant Dialog - полная версия как основное модальное окно */}
      {perms.products && (
          ...
          orders.map((o) => (
            <div key={o._id} className="grid grid-cols-12 gap-2 px-3 py-3 border-t border-gray-200 dark:border-white/10 text-sm items-center hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
              ...
              <div className="col-span-1 flex justify-end">
                <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 text-gray-700 dark:text-white" onClick={() => { setSelected(o); setViewOpen(true); }}>
...
function ProductsList({ products }) {
  if (!products?.length) return <div className="text-white/60">Mahsulotlar yo'q</div>;
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {products.map((p) => (
        <div key={p._id} className="rounded-xl border border-white/10 bg-white/5 p-3">
          <div className="font-semibold">{p.name}</div>
          <div className="text-white/70 text-sm">Narx: {p.price} {p.currency}</div>
        </div>
      ))}
    </div>
  );
